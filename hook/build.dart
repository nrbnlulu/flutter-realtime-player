...
const _pkgConfigSysrootArmv7EnvVar =
    'PKG_CONFIG_SYSROOT_DIR_armv7_linux_androideabi';
...
void main(List<String> args) async {
  ...
  await build(args, (input, output) async {
    if (input.config.buildCodeAssets) {
      final gstreamerRoot = envFile.getString(_prebuiltStreamerRootEnvVar);
      final androidPkgConfigPath = _androidPkgConfigPath(
        input.config.code,
        gstreamerRoot,
      );
...
String? _androidPkgConfigPath(CodeConfig config, String gstreamerRoot) {
  if (config.targetOS != OS.android || gstreamerRoot.isEmpty) {
    return null;
  }

  final gstArch = switch (config.targetArchitecture) {
  ...
  return '$gstreamerRoot/$gstArch/lib/pkgconfig';
}
