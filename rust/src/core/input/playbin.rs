...
#[cfg(target_os = "android")]
struct PlaybinVideoOutput {
    output: Arc<AndroidVideoOutput>,
}

#[cfg(target_os = "android")]
impl PlaybinVideoOutput {
    fn new(engine_handle: i64) -> anyhow::Result<Self> {
        Ok(Self {
    ...
    fn make_video_sink(&self, session: &Arc<PlaybinSession>) -> anyhow::Result<gst::Element> {
        let glimagesink = gst::ElementFactory::make("glimagesink")
            .name("sink")
            .property("sync", false)
            .build()
            .context("Failed to create glimagesink element")?;

        set_window_handle(&glimagesink, &self.output)?;
...
impl PlaybinVideoOutput {
    ...
    fn make_video_sink(&self, session: &Arc<PlaybinSession>) -> anyhow::Result<gst::Element> {
        let texture_session = Arc::new(crate::core::texture::flutter::TextureSession::new(
            self.texture_id,
            Arc::downgrade(&self.sendable_texture),
            self.payload_holder_weak.clone(),
        ));
...
impl PlaybinSession {
    ...
    pub async fn execute(
        ...
        let video_output = PlaybinVideoOutput::new(self.session_common.engine_handle)?;
        let texture_id = video_output.texture_id();

        // This will ensure that we only destroy the texture on platform main thread
        let inner_result = self
            .run_pipeline(shutdown_rx, &video_output, texture_id)
            .await;
    ...
    async fn run_pipeline(
        ...
        video_output: &PlaybinVideoOutput,
        texture_id: i64,
    ) -> anyhow::Result<()> {
        self.session_common.send_state_msg(StreamState::Loading);
        let video_sink = video_output.make_video_sink(self)?;
