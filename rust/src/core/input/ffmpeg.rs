...
impl FfmpegVideoSession {
    ...
    pub fn initialize_stream(&self) -> Result<(), ffmpeg::Error> {
        ...
        if decoder_width == 0 || decoder_height == 0 {
            warn!(
                "Decoder dimensions unknown during init ({}x{}); will process at native resolution",
        ...
        loop {
            match ctx.decoder.receive_frame(&mut decoded) {
                ...
                Err(ffmpeg::Error::InputChanged) => {
                    ...
                    // Flush decoder to adapt to new stream
            ...
            // Convert to RGBA format at native resolution (no scaling)
            ...
            if let Err(e) = scaler.run(&decoded, &mut rgb_frame) {
                warn!("Scaler error during native resolution conversion: {}", e);
                continue;
            }
