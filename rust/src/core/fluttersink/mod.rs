...
static ref SESSION_CACHE: Mutex<HashMap<i64, (Arc<SoftwareDecoder>, i64)>> = Mutex::new(HashMap::new());
}
...
pub fn destroy_engine_streams(engine_handle: i64) {
    ...
    for (texture_id, (decoder, handle)) in session_cache.iter() {
...
pub fn destroy_stream_session(texture_id: i64) {
    ...
    let mut session_cache = SESSION_CACHE.lock().unwrap();
    if let Some((decoder, _)) = session_cache.remove(&texture_id) {
        decoder.destroy_stream();
